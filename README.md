...
Il progetto Over the Rainbow affronta il problema critico del Controllo Qualità (QC) nei VFX, dove il denoising post-produzione introduce difetti difficili da tracciare, come noise residuo e artefatti (glitch, flickering). Il nostro obiettivo di classificazione binaria è determinare se un frame è "Pulito e Accettabile" (Classe 0) o "Contaminato da Artefatti (Classe 1). Per garantire la massima accuratezza e robustezza, proponiamo l'utilizzo della funzione "Singularity Machine Learning - Classification" di Multiverse Computing, un Quantum-Enhanced Ensemble Classifier (QEEC) basato su Qiskit. Questa soluzione è ideale perché sfrutta l'ottimizzazione quantistica QAOA per trovare la combinazione ottimale di weak learners, permettendo di distinguere con precisione feature complesse come gli artefatti. Inoltre, gestisce efficacemente feature vector ad alta dimensionalità derivati da milioni di frame senza limitazioni di qubit per la dimensione dei dati. Questo approccio garantisce un beneficio industriale diretto, alleggerendo il carico degli artisti, riducendo drasticamente il tempo necessario per il QC e accelerando il throughput complessivo della produzione cinematografica.
